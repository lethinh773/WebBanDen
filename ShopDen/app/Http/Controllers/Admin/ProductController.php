...
class ProductController extends Controller
{
    ...
    //     // $image_resize = Image::make($requestImage->getPathname());
    ...
    public function addImageProduct($product, $requestImage, $primary = false)
{
    ...
    // Lấy thông tin về tệp và đường dẫn lưu trữ
    $file = $requestImage->getClientOriginalName();
    $fileName = pathinfo($file, PATHINFO_FILENAME);
    $extension = $requestImage->getClientOriginalExtension();
    $imageName = $fileName . "_" . $product->id . "_" . $image->id . "." . $extension;

    // Di chuyển tệp vào thư mục đích
    $requestImage->move(public_path('images/product'), $imageName);

    // Lưu thông tin đường dẫn vào cơ sở dữ liệu
    $image->path = '/images/product/' . $imageName;
    $image->name = $imageName;
    $image->save();
}
